...
export function formatControlHelp(machineName: string): MessageContent {
  const text = [
    ...
    '`/wm delete <name>` — Delete a project',
    '`/wm list` — List all projects',
    '`/wm config <project> <field> <value>` — Update project config',
    '',
    '*Machine*',
    '`/wm status` — Show machine status',
    '`/wm models` — List available models',
    '`/wm sessions` — List all Copilot CLI sessions',
    '`/wm join [session-id]` — Join an existing session (dropdown if no ID)',
    '`/wm help` — Show this help',
    '',
    '_In a project channel, commands apply to that project directly._',
  ].join('\n');
...
export function formatProjectHelp(projectName: string): MessageContent {
  const text = [
    ...
    '`/wm sessions` — List all Copilot CLI sessions',
    '`/wm join <session-id>` — Join an existing session',
    '`/wm leave` — Detach without killing the session',
    '`/wm history` — Show session history',
    '`/wm diff` — Show git diff',
    '',
    '*Config*',
    '`/wm model <model-name>` — Change the model',
    '`/wm agent <agent-name>` — Change the agent',
    '`/wm mode [normal|plan|autopilot]` — Show or change mode',
    '`/wm help` — Show this help',
    '',
    '_Or just type a message to send a prompt to Copilot._',
  ].join('\n');
